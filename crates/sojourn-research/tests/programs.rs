...
fn facility_gate_blocks_a_step() {
    ...
    assert_eq!(
        snap(&core, &m).maturity(FactionId(0), &b15).unwrap().trl,
        6,
        "blocked at TRL 6 without the facility"
    );
    ...
    advance_days(&mut core, 365 * 3);
    assert!(
        snap(&core, &m).maturity(FactionId(0), &b15).unwrap().trl > 6,
        "advances once the facility is available"
    );
}
...
fn heritage_raises_reliability_and_enables_a_derivative() {
    ...
    let before = snap(&core, &m)
        .maturity(FactionId(0), &b15)
        .unwrap()
        .reliability;
    research(
        &mut core,
    ...
    let after = snap(&core, &m)
        .maturity(FactionId(0), &b15)
        .unwrap()
        .reliability;
    assert!(
        after > before,
        "heritage raised reliability ({before:.3} → {after:.3})"
    );
    ...
    assert!(
        mat.trl >= 7,
        "derivative starts partway up the ladder (TRL {})",
        mat.trl
    );
}
