...
fn tide_advances_and_publishing_accelerates_it() {
    ...
    assert!(
        w_hold > 0.0,
        "tide advances from aggregate + baseline even when holding ({w_hold:.3})"
    );
    assert!(
        w_pub > w_hold,
        "publishing accelerates World UL ({w_pub:.3} > {w_hold:.3})"
    );
}
...
fn publish_emits_a_prestige_eligible_event_and_private_leads_world() {
    ...
    assert!(
        count_events(&core, "publish") >= 1,
        "publish emits an event (FA-09 prestige hook)"
    );
    let s = snap(&core, &m);
    let priv_ul = s
        .understanding(FactionId(0), &DomainId("A2".into()))
        .unwrap()
        .private_ul;
    let world = s.tide(&DomainId("A2".into())).unwrap();
    assert!(
        priv_ul >= world,
        "a faction's private UL = world + lead ({priv_ul:.2} ≥ {world:.2})"
    );
}
