...
fn pipeline_trains_to_ready_then_crew_feedback_retires_over_limit() {
    ...
    research(&mut core, Hire { faction: 0, role: Role::Astronaut, discipline: DomainId("A13".into()), skill: 5.0, traits: vec![] });
    assert_eq!(snap(&core, &m).personnel(FactionId(0))["AstronautReady"], 0, "candidate is not yet ready");

    // Train (730 days) → Ready.
    research(&mut core, Train { person: 0 });
    advance_days(&mut core, 800);
    assert_eq!(snap(&core, &m).personnel(FactionId(0))["AstronautReady"], 1, "training completes → ready");

    // FA-08 in-mission feedback over the career dose limit → leaves the ready pool.
    research(&mut core, CrewFeedback {
        faction: 0,
        astronaut: 0,
    ...
    assert_eq!(snap(&core, &m).personnel(FactionId(0))["AstronautReady"], 0, "over the dose limit → retired");
}
...
fn training_requires_the_time_gate() {
    ...
    assert_eq!(snap(&core, &m).personnel(FactionId(0))["AstronautReady"], 0, "not ready before the training gate");
}
