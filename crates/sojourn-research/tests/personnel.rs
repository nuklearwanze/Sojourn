...
fn tacit_knowledge_lowers_effective_ul_without_touching_ground_ul() {
    ...
    assert!(
        (u1.private_ul - u1.effective_ul).abs() < 1e-9,
        "specialist present ⇒ no tacit penalty"
    );
    ...
    assert!(
        u2.effective_ul < u2.private_ul,
        "tacit-knowledge loss lowers effective UL"
    );
    assert!(
        (u2.private_ul - u2.effective_ul - 12.0).abs() < 1e-6,
        "penalty = tacit_per_head (12)"
    );
    ...
    assert!(
        (u3.private_ul - u3.effective_ul).abs() < 1e-9,
        "re-hiring restores effective UL"
    );
}
