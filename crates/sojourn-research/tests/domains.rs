...
fn understanding_grows_and_gates_engineering() {
    ...
    assert!(
        u.private_ul > 60.0,
        "UL grew past the floor ({:.1})",
        u.private_ul
    );
    assert!(u.private_ul <= 100.0);
    assert!(
        s1.available_programs(FactionId(0)).contains(&b15),
        "B1.5 now available"
    );
}
...
fn diminishing_returns_slow_growth_near_the_knee() {
    ...
    let early = snap(&core, &m)
        .understanding(FactionId(0), &a2)
        .unwrap()
        .private_ul;
    ...
    assert!(
        gain_high < gain_low,
        "diminishing returns: {gain_high:.2} (high) < {gain_low:.2} (low)"
    );
}
